require 'flint/commands_generator'

describe Flint::CommandsGenerator do
  let(:available_options) { Flint::Options.available_options }

  def expect_runner_run_with(expected_options)
    fake_runner = "runner"
    expect(Flint::Runner).to receive(:new).and_return(fake_runner)
    expect(fake_runner).to receive(:run) do |actual_options|
      expect(expected_options._values).to eq(actual_options._values)
    end
  end

  Flint.environments.each do |env|
  ...
  describe ":change_password option handling" do
    def expect_change_password_with(expected_options)
      expect(Flint::ChangePassword).to receive(:update) do |args|
  ...
  describe ":decrypt option handling" do
    def expect_githelper_clone_with(git_url, shallow_clone, git_branch)
      expect(Flint::GitHelper).to receive(:clone).with(git_url, shallow_clone, git_branch)
  ...
  def expect_nuke_run_with(expected_options, type)
    fake_nuke = "nuke"
    expect(Flint::Nuke).to receive(:new).and_return(fake_nuke)
